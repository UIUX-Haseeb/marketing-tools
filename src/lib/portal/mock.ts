/**
 * Mock Portal — used until PORTAL_API_URL / PORTAL_API_KEY are configured.
 * Same shape as the real client so the tools cannot tell the difference.
 * Photos are generated SVG portraits (initials on a brand colour).
 */
...
const PEOPLE: Array<[string, string, string, boolean]> = [
  ...
  ["Zainab Hussain", "Executive Assistant", null as unknown as string, true],
];

const employees: Employee[] = PEOPLE.map(([fullName, designation, dob, hasPhoto], i) => ({
  id: `mock-${i + 1}`,
  fullName,
  designation,
...
function portraitSvg(e: Employee, i: number) {
  const initials = e.fullName.split(/\s+/).slice(0, 2).map((s) => s[0]).join("");
  ...
  <circle cx="500" cy="380" r="170" fill="#FAF8F4" fill-opacity="0.18"/>
  <ellipse cx="500" cy="900" rx="330" ry="290" fill="#FAF8F4" fill-opacity="0.18"/>
  <text x="500" y="560" text-anchor="middle" font-family="Arial, sans-serif" font-size="180" font-weight="600" fill="#FAF8F4" fill-opacity="0.9">${initials}</text>
</svg>`;
}

export const mockPortal: PortalClient = {
  async listEmployees() {
  ...
  async getPhoto(id) {
    ...
    if (i < 0 || !employees[i].hasPhoto) return null;
    const svg = portraitSvg(employees[i], i);
    return { bytes: new TextEncoder().encode(svg).buffer as ArrayBuffer, contentType: "image/svg+xml" };
  },
};
