...
**Assets:** the two PNGs in `public/tools/promotion/` are code-generated stand-ins. Export the real layers from Figma at 1× (1152×1440) and replace them, same file names: `background.png` = bg + gradients + circle pattern only (no text layers, no logo, no script, no frame — all of those are drawn by code); `frame.png` = the "profile frame" layer alone on a transparent canvas.
